...
def addfavorite(request, id): #todo product classının içinde addfavorite tanımlayarak dene
    url = request.META.get('HTTP_REFERER')
    current_user = request.user
    checkinproduct = Favorite.objects.filter(product_id=id, user_id=current_user.id).first()
    if checkinproduct:
